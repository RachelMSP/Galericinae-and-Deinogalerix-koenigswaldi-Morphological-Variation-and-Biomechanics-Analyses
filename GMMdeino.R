#GPA, PCA, PCA PLOTS + MESH DEFORMATIONS, ANOVAS, CAC, CAC PLOT + MESH DEFORMATIONS >> FOR GYMNURE AND DEINOGALERIX DATASET 

rm(list=ls()) 
setwd("/Users/raquelcardenas/Desktop/prog/gmmdeino")
...
for (file in specimen_files) {
  ...
  landmark_matrix[landmark_matrix > 1e6]
...
species_factor <- factor(species_codes,
                         levels = c("Eg", "Hs", "Ns", "Om", "Pt", "Dk"),  #Add "Di" with D.intermedius inclusion  
                         labels = c("Echinosorex gymnura", "Hylomys suillus", "Neotetracus sinensis", "Otohylomys megalotis", "Podogymnura truei", "Deinogalerix koenigswaldi"))  # add "Deinogalerix intermedius" with D.i inclusion 
...
levels(species_colors) <- c("coral1", "goldenrod1", "chartreuse1", "blueviolet", "deepskyblue", "blue")  # add "hotpink" with D.i inclusion
...
# Filtering the landmark array to remove landmarks that are missing/NAs in Deinogalerix specimens
landmarks_to_keep <- setdiff(1:dim(landmark_array)[1], c(14, 13, 15)) #add 1, 2, 3, 16 with D.i inclusion
landmark_array_filtered <- landmark_array[landmarks_to_keep, , ]


#SECTION 2 - GPA RESULTS
gpa_results <- gpagen(landmark_array_filtered, ProcD = TRUE)


#SECTION 3 - MESH DEFORMATIONS ALONG PCA
ProcSym_MT = procSym(landmark_array_filtered)
which.min(ProcSym_MT$rho)

# Finding speciemen closest to the mean
min_rho_index <- which.min(ProcSym_MT$rho)
closest_landmarks <- landmark_array_filtered[, , min_rho_index]
cat(sprintf("Closest specimen is: %s\n", specimen_names[min_rho_index]))

sur.clo <- file2mesh("~/Desktop/prog/meshes_gymnures/M035HsML.stl")
#before it was 35(with 057), other test was25
...
vcgPlyWrite(mean_mesh, filename = "~/Desktop/prog/meshes_gymnures/newgymnure_mean.ply")

# Mean shape
pcscores <- ProcSym_MT$PCscores
pcs <- ProcSym_MT$PCs
output_dir <- "~/Desktop/NEW3D_Warped_Models/"
...
pca_results <- procSym(landmark_array_filtered)
pca_results$rotated

# Data frame
PC_data <- as.data.frame(pca_results$PCscores)
PC_data$specimen <- specimen_names
...
# Plotting PCA Plor
xlim_range <- c(min(pcs[, 1]) - 0.03, max(pcs[, 1]) + 0.05)
ylim_range <- c(min(pcs[, 2]) - 0.08, max(pcs[, 2]) + 0.045)
#xlim_range <- c(min(pcs[, 1]) - 0.020, -(min(pcs[, 1]) - 0.020))
#ylim_range <- c(-(max(pcs[, 2]) + 0.04), max(pcs[, 2]) + 0.04)
plot(pcs[, 1], pcs[, 2],
     ...
     main = "PCA - Galericinae (extant species and  D. koenigswaldi )",
     cex.main = 0.9,
     xlim = xlim_range,
     ylim = ylim_range)
...
# Legend
legend("topright", legend = levels(species_factor) , col = levels(species_colors), cex = 0.70, text.font = 3, pch = 16, title = "Galericinae Species")
...
ref_mesh <- file2mesh("~/Desktop/prog/meshes_gymnures/M008EgML.stl")
...
shade3d(mesh_small,col = "darkgrey", add = T)
shade3d(mesh_large,col = "darkgrey", add = T)

# Exporting
output_dir <- "~/Desktop/FINAL3DD_Warped_Models_CAC"
dir.create(output_dir, showWarnings = FALSE)
